    ...
    render() {
        var exp = [
            {
                ...
                desc: 'I’m currently in my third year of the Software Engineering program at Carleton. It has provided me with comprehensive knowledge regarding fundamental computing theory and practice, processes, methods and tools for developing software systems, and regulatory and social issues. Emphasis is placed on developing expertise in object-oriented programming (using C++ and Java) and real-time computer systems.'
            }
        ]
        var exp2 = [
            {
        ...
        return (
            <div>
                ...
                {exp.map((exp) => this.renderExperience(exp.place, exp.title, exp.date, exp.desc))}
                <Container className='experience_cont'>
                    <Row className='skills_line_row_title'>
                        <Col className='light_text experience_title' md='6'>
                            <h3>Experience</h3>
                ...
                {exp2.map((exp) => this.renderExperience(exp.place, exp.title, exp.date, exp.desc))}
            </div>
